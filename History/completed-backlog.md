...
### Sprint 13: desktop-нода стала понятнее как torrent/swarm программа

- local desktop node теперь показывает не только `daemon/gateway` статус, но и:
  - лимиты места и канала
  - очередь новых bags для хранения
  - живые сессии раздачи
  - inventory bags
- добавлено локальное сохранение настроек ноды и local route для их изменения
- remote desktop UI теперь может читать и обновлять local node settings, даже если сама страница открыта с удалённого origin

### Sprint 13: студия больше не упирается в demo preview как в ручную ловушку

- после загрузки master-файла трека студия автоматически пытается создать demo preview MP3
- ручная загрузка preview осталась только как замена автогенерированного demo
- single-релиз с одним треком может подхватить demo прямо из общего master upload
- ошибка создания релиза теперь говорит, какой именно трек ещё не готов, а не просто валится общей фразой
