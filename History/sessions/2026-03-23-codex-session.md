...
## Следующий TON Storage bridge slice

- добавлен server-side bridge status для:
  - upload mode
  - `storage-daemon-cli`
  - HTTP gateway retrieval
- runtime fetch теперь умеет резолвить реальные `tonstorage://<BagID>/...` pointers через настроенный gateway
- локальный worker получил режим `tonstorage_cli`, который может вернуть уже настоящий `BagID`

## Следующий runtime probe slice

- в админке storage появился `Runtime probe`
- теперь operator может проверить конкретный asset/bag и увидеть:
  - через какой runtime path идёт резолв
  - отвечает ли источник уже как HTTP file target
  - какой статус и content-type вернулись

## Desktop node map preview

- desktop-экран `C3K Desktop Client` получил карту swarm-нод на реальной open-source географии
- теперь пользователь видит не только onboarding, но и:
  - свою desktop-ноду
  - gateway для `c3k.ton`
