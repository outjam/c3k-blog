  ...
  return (
    <>
      <motion.div
        ...
        transition={{ type: "spring", stiffness: 380, damping: 32, mass: 0.66 }}
      >
        <motion.article className={styles.miniCard} layoutId="global-player-card" whileTap={{ scale: 0.995 }}>
          <button type="button" className={styles.miniMain} onClick={() => setExpanded(true)}>
            {currentTrack.coverUrl ? (
              <Image src={currentTrack.coverUrl} alt={currentTrack.title} width={44} height={44} className={styles.coverSmall} />
            ) : (
          ...
          <div className={styles.miniActions}>
            ...
            <button type="button" className={styles.playMainMini} onClick={togglePlayback} aria-label={isPlaying ? "Пауза" : "Воспроизвести"}>
              {isPlaying ? "❚❚" : "▶"}
            </button>
      ...
      <AnimatePresence>
        {expanded ? (
          <motion.div className={styles.fullOverlay} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.button
              type="button"
              ...
              aria-label="Закрыть полный плеер"
            />

            <motion.section
              className={styles.fullSheet}
              layoutId="global-player-card"
              initial={{ y: "108%", opacity: 0.74 }}
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: "108%", opacity: 0.75 }}
            ...
            >
              <button type="button" className={styles.handleButton} onClick={closeExpanded} aria-label="Свернуть плеер">
                <span />
              </button>
              ...
              <AnimatePresence initial={false}>
                {queueOpen ? (
                  <motion.div
                    ...
                    transition={{ type: "spring", stiffness: 380, damping: 32, mass: 0.63 }}
                  >
